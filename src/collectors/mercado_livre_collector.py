from __future__ import annotations

import os

from src.collectors.base_collector import BaseCollector, SourceContext
from src.processing.price_parser import PriceObservationIn

ML_SEARCH_API = "https://api.mercadolibre.com/sites/MLB/search"

PRODUCT_TERMS = {
    "rolinho_galvalume": (("galvalume",), ("rolinho", "rolo", "bobina")),
    "rolinho_galvanizado_zincado_fosco": (("fosco",), ("rolinho", "rolo", "bobina")),
    "rolinho_galvanizado_zincado_brilhante": (("brilhante",), ("rolinho", "rolo", "bobina")),
    "telha_termoacustica_sanduiche_trapezio": (("telha",), ("sanduiche", "térmica", "termoacustica")),
    "divisoria_gesso_drywall": (("drywall",), ("placa", "chapa", "divisoria")),
    "perfil_montante_drywall": (("montante", "drywall"), ()),
    "perfil_guia_drywall": (("guia", "drywall"), ()),
}


def is_comparable_result(product_id: str, title: str | None) -> bool:
    """Keep only new listings whose titles describe the monitored product."""
    normalized = (title or "").casefold()
    required, alternatives = PRODUCT_TERMS.get(product_id, ((), ()))
    if not all(term in normalized for term in required):
        return False
    return not alternatives or any(term in normalized for term in alternatives)


class MercadoLivreCollector(BaseCollector):
    def collect(self, context: SourceContext) -> list[PriceObservationIn]:
        query = context.query or context.product_id
        try:
            self.cooldown(ML_SEARCH_API)
            headers = {}
            token = os.getenv("MERCADOLIVRE_ACCESS_TOKEN")
            if token:
                headers["Authorization"] = f"Bearer {token}"
            response = self.session.get(
                ML_SEARCH_API,
                params={"q": query, "limit": 5},
                headers=headers,
                timeout=25,
            )
            if response.status_code in {403, 429}:
                hint = " Configure MERCADOLIVRE_ACCESS_TOKEN." if response.status_code == 403 else ""
                return [self.status_observation(context, "blocked", f"API retornou HTTP {response.status_code}.{hint}")]
            response.raise_for_status()
            data = response.json()
        except Exception as exc:
            return [self.status_observation(context, "error", f"Erro na API Mercado Livre: {exc}")]

        observations: list[PriceObservationIn] = []
        for item in data.get("results", []):
            if item.get("condition") != "new" or not is_comparable_result(context.product_id, item.get("title")):
                continue
            seller = item.get("seller", {}).get("nickname", "vendedor nao informado")
            observations.append(
                PriceObservationIn(
                    competitor_id=context.competitor_id,
                    competitor_name=f"Mercado Livre - {seller}",
                    tier=context.tier,
                    product_id=context.product_id,
                    category=context.category,
                    url=item.get("permalink") or context.url,
                    raw_product_name=item.get("title"),
                    raw_price=f"R$ {float(item.get('price', 0)):.2f}",
                    parsed_price=float(item.get("price", 0)),
                    raw_unit="peca",
                    availability=f"Quantidade disponivel: {item.get('available_quantity')}",
                    collection_status="success",
                    source_type="marketplace",
                    customer_segment=context.customer_segment,
                    unit_assumed=True,
                ).finalized()
            )
            if len(observations) == 5:
                break
        if not observations:
            observations.append(
                self.status_observation(
                    context,
                    "unavailable",
                    f"Nenhum anuncio novo e comparavel retornado pela API para a busca: {query}",
                    source_type="marketplace",
                )
            )
        return observations
